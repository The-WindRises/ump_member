...
public class Updater {
    ...
    public void download(UpdaterConfig updaterConfig) {
        ...
        Log.i(TAG, "local download id is " + downloadId);
        if (downloadId != -1L) {
            ...
            switch (status) {
                ...
                case DownloadManager.STATUS_FAILED:
                    Log.i(TAG, "download failed " + downloadId);
                    startDownload(updaterConfig);
                    break;
                case DownloadManager.STATUS_RUNNING:
                    Log.i(TAG, "downloadId=" + downloadId + " ,status = STATUS_RUNNING");
                    break;
                case DownloadManager.STATUS_PENDING:
                    Log.i(TAG, "downloadId=" + downloadId + " ,status = STATUS_PENDING");
                    break;
                case DownloadManager.STATUS_PAUSED:

                    Log.i(TAG, "downloadId=" + downloadId + " ,status = STATUS_PAUSED");
                    break;
                case STATUS_UN_FIND:
                    Log.i(TAG, "downloadId=" + downloadId + " ,status = STATUS_UN_FIND");
                    startDownload(updaterConfig);
                    break;
                default:
                    Log.i(TAG, "downloadId=" + downloadId + " ,status = " + status);
                    break;
            }
        } else {
            startDownload(updaterConfig);
    ...
    private void startDownload(UpdaterConfig updaterConfig) {
        ...
        Log.i(TAG, "apk download start, downloadId is " + id);
    }
